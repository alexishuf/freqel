...
public class IdRefHashMap<K, V> extends AbstractMap<K, V> implements RefMap<K, V> {
    ...
    private void alloc(int buckets, int bucketSize, @Nullable Object[] keys,
        ...
        if (keys != null) {
            ...
            for (int i = 0; i < keys.length; i++) {
                Object key = keys[i];
                if (key != null) {
                    int begin = (hash(key) & bucketMask) * this.bucketSize;
                    boolean ok = false;
    ...
    public IdRefHashMap(int buckets, int bucketSize) {
        alloc(buckets, bucketSize, null, null);
    }

    public IdRefHashMap(int capacity) {
        alloc(capacity,  3, null, null);
    }

    public IdRefHashMap(@Nonnull Map<K, V> other) {
        this(other.size(), 3);
    ...
    @Override public V get(Object key) {
        ...
        int begin = (hash(key) & bucketMask) * bucketSize;
        for (int i = begin, end = begin+bucketSize; i < end; i++) {
    ...
    private abstract class BaseIterator<T> implements Iterator<T> {
        ...
        @Override public void remove() {
            ...
            IdRefHashMap.this.remove(lastIndex, (lastIndex/bucketSize + 1) * bucketSize);
            index = lastIndex-1;
            advance();
        }
    }
    ...
    private final class EntrySet extends AbstractSet<Entry<K, V>> {
        ...
        @Override public boolean contains(Object o) {
            @SuppressWarnings("unchecked") Entry<K, V> e = (Entry<K, V>) o;
            V v = IdRefHashMap.this.get(e.getKey());
            return v != null && v.equals(e.getValue());
        }

        @Override public boolean add(Entry<K, V> e) {
            V old = IdRefHashMap.this.put(e.getKey(), e.getValue());
            return !Objects.equals(old, e.getValue());
        }

        @Override public boolean remove(Object o) {
            @SuppressWarnings("unchecked") Entry<K, V> e = (Entry<K, V>) o;
            return IdRefHashMap.this.remove(e.getKey(), e.getValue());
        }

        @Override public void clear() {
            IdRefHashMap.this.clear();
        }

        @Override public int hashCode() {
            return IdRefHashMap.this.hashCode();
        }
    }
    ...
    private final class KeySet extends AbstractSet<K> {
        ...
        @Override public boolean remove(Object o) {
            return IdRefHashMap.this.remove(o) != null;
        }
