...
public class RefHashMapSimpleGrowth<K, V> extends AbstractMap<K, V> implements RefMap<K, V> {
    ...
    public RefHashMapSimpleGrowth(int buckets, int bucketSize) {
        alloc(buckets, bucketSize, null, null);
    }

    public RefHashMapSimpleGrowth(int capacity) {
        alloc(capacity/2 + 1,  3, null, null);
    }

    public RefHashMapSimpleGrowth(@Nonnull Map<K, V> other) {
    ...
    private class EntryIterator implements Iterator<Entry<K, V>> {
        ...
        @Override public void remove() {
            ...
            RefHashMapSimpleGrowth.this.remove(lastIndex, (lastIndex/bucketSize + 1) * bucketSize);
            index = lastIndex-1;
            advance();
        }
    }
    ...
    private class EntrySet extends AbstractSet<Entry<K, V>> {
        ...
        @Override public boolean contains(Object o) {
            @SuppressWarnings("unchecked") Entry<K, V> e = (Entry<K, V>) o;
            V v = RefHashMapSimpleGrowth.this.get(e.getKey());
            return v != null && v.equals(e.getValue());
        }

        @Override public boolean add(Entry<K, V> e) {
            V old = RefHashMapSimpleGrowth.this.put(e.getKey(), e.getValue());
            return !Objects.equals(old, e.getValue());
        }

        @Override public boolean remove(Object o) {
            @SuppressWarnings("unchecked") Entry<K, V> e = (Entry<K, V>) o;
            return RefHashMapSimpleGrowth.this.remove(e.getKey(), e.getValue());
        }

        @Override public void clear() {
            RefHashMapSimpleGrowth.this.clear();
        }

        @Override public int hashCode() {
            return RefHashMapSimpleGrowth.this.hashCode();
        }
    }
}
