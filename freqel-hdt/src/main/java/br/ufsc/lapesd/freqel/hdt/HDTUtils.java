package br.ufsc.lapesd.freqel.hdt;
...
public class HDTUtils {
    ...
    public static @Nonnull Term toTerm(@Nonnull CharSequence cs,
        ...
        if (first == '"' || Character.isDigit(first)) {
            Term term = RDFUtils.fromNT(string, termFac);
            assert term != null;
            return term;
        }
    ...
    public static @Nonnull Stream<Term> streamTerm(@Nonnull Iterator<TripleString> it,
                                                   @Nonnull br.ufsc.lapesd.freqel.model.Triple.Position position) {
        Iterator<Term> termIt = new Iterator<Term>() {
            ...
            @Override public boolean hasNext() {
                while (next == null && it.hasNext()) {
                    ...
                    CharSequence cs;
                    if (position == br.ufsc.lapesd.freqel.model.Triple.Position.SUBJ)
                        cs = ts.getSubject();
                    else if (position == br.ufsc.lapesd.freqel.model.Triple.Position.PRED)
                        cs = ts.getPredicate();
                    else
                        cs = ts.getObject();
                    try {
