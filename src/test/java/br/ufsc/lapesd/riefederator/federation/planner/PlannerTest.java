...
public class PlannerTest {
    ...
    public void testDuplicateQuery(@Nonnull Supplier<Planner> supplier) {
        ...
        assertTrue(TreeUtils.streamDepthLeft(root).count() <= 3);
    }
    ...
    public void testSingleJoin(@Nonnull Supplier<Planner> supplier) {
        ...
        assertTrue(TreeUtils.streamDepthLeft(root).count() <= 5);
        List<JoinNode> joins = TreeUtils.streamDepthLeft(root).filter(n -> n instanceof JoinNode)
    ...
    public void testCartesianProduct(@Nonnull Supplier<Planner> supplier) {
        ...
        assertTrue(TreeUtils.streamDepthLeft(root).count() <= 5);
        List<CartesianNode> nodes = TreeUtils.streamDepthLeft(root)
    ...
    public void testLargeTree(@Nonnull Supplier<Planner> supplier) {
        ...
        for (int i = 0; i < 16; i++) {
            ...
            Set<QueryNode> observed = TreeUtils.streamDepthLeft(root)
                                               .filter(n -> n instanceof QueryNode)
                                               .map(n -> (QueryNode) n).collect(toSet());
            assertEquals(observed, new HashSet<>(shuffled));
            List<JoinNode> joinNodes = TreeUtils.streamDepthLeft(root)
            ...
            List<CartesianNode> cartesianNodes = TreeUtils.streamDepthLeft(root)
                                                          .filter(n -> n instanceof CartesianNode)
