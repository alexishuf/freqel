...
public class ModifiersSet extends AbstractSet<Modifier> {
    ...
    private boolean mergeNonProjection(@Nonnull Modifier modifier, boolean change) {
        if (modifier instanceof ValuesModifier) {
            ...
            if (mine != null) {
                change |= add(joinValues(mine, theirs));
            } else {
                change |= add(theirs);
            }
    ...
    private @Nonnull ValuesModifier joinValues(@Nonnull ValuesModifier left,
                                               @Nonnull ValuesModifier right) {
        IndexSet<String> all = FullIndexSet.fromDistinctCopy(left.getVarNames());
        all.addAll(right.getVarNames());
        Set<String> shared = intersect(left.getVarNames(), right.getVarNames());
        int worstCase = left.getAssignments().size() * right.getAssignments().size();
        List<Solution> solutions = new ArrayList<>(Math.min(2048, worstCase));
        ArraySolution.ValueFactory factory = ArraySolution.forVars(all);
        for (Solution ls : left.getAssignments()) {
            for (Solution rs : right.getAssignments()) {
                boolean match = true;
                for (String var : shared) {
                    if (!(match = Objects.equals(ls.get(var), rs.get(var))))
                        break;
                }
                if (match)
                    solutions.add(factory.fromSolutions(ls, rs));
            }
        }
        return new ValuesModifier(all, solutions);
    }
