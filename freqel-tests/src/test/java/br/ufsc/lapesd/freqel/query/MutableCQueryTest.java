...
import static br.ufsc.lapesd.freqel.model.Triple.Position.*;
...
public class MutableCQueryTest implements TestContext {
    ...
    public void testMultipleFilters() {
        ...
        assertTrue(q.mutateModifiers().add(SPARQLFilterFactory.parseFilter("?y > 23")));
        assertTrue(q.mutateModifiers().add(SPARQLFilterFactory.parseFilter("?y < 35")));
        assertFalse(q.mutateModifiers().add(SPARQLFilterFactory.parseFilter("?y < 35")));
        assertEquals(q.getModifiers(),
                newHashSet(SPARQLFilterFactory.parseFilter("?y > 23"), SPARQLFilterFactory.parseFilter("?y < 35")));
    ...
    public void testFilterWithInput() {
        ...
        assertTrue(q.mutateModifiers().add(SPARQLFilterFactory.parseFilter("?u < 23")));
        assertEquals(q.getModifiers().filters(), singleton(SPARQLFilterFactory.parseFilter("?u < 23")));
    ...
    public void testSanitizeFilters() {
        ...
        q.add(new Triple(x, knows, y));
        q.add(new Triple(y, age, u));
        q.mutateModifiers().add(SPARQLFilterFactory.parseFilter("?u > 23"));
        q.mutateModifiers().add(Ask.INSTANCE);

        assertEquals(q.sanitizeFilters(), emptySet());
        heatCache(q);
        q.remove(1);
        assertEquals(q.sanitizeFilters(), singleton(SPARQLFilterFactory.parseFilter("?u > 23")));
        checkCache(q);
    ...
    public void testSanitizeFilterStrictly() {
        ...
        q.add(new Triple(x, knows, y));
        q.add(new Triple(y, age, u));
        q.mutateModifiers().add(SPARQLFilterFactory.parseFilter("?u > ?v"));
        ...
        assertEquals(q.sanitizeFiltersStrict(), singleton(SPARQLFilterFactory.parseFilter("?u > ?v")));
        checkCache(q);
        ...
        q.mutateModifiers().add(SPARQLFilterFactory.parseFilter("?u > ?v")); // re-add the filter
        ...
        heatCache(q);
        assertEquals(q.sanitizeFilters(), singleton(SPARQLFilterFactory.parseFilter("?u > ?v")));
        checkCache(q);
    ...
    public void testSanitizeProjectionStrict() {
        MutableCQuery q = generateConstellationQuery(3);
        assertTrue(q.mutateModifiers().add(Projection.of("o1", "y")));
        assertTrue(q.mutateModifiers().add(SPARQLFilterFactory.parseFilter("?o1 > ?y")));
        assertEquals(q.attr().publicVarNames(), newHashSet("o1", "y"));
        ...
        MutableCQuery expected = generateConstellationQuery(3);
        expected.mutateModifiers().add(Projection.of("o1"));
        expected.mutateModifiers().add(SPARQLFilterFactory.parseFilter("?o1 > ?y"));
        assertEqualQueries(q, expected);
    }
