...
public class JoinOpTest implements TestContext {
    ...
    public void testDeepCopyPreservesUniverseSets() {
        QueryOp left = new QueryOp(createQuery(
                Alice, knows, x, Alice, age, u, SPARQLFilterFactory.parseFilter("?u < ?v")));
        QueryOp right = new QueryOp(createQuery(x, age, v, SPARQLFilterFactory.parseFilter("?v > 23")));
        ...
        SPARQLAssert.assertUniverses(left);
        SPARQLAssert.assertUniverses(right);
        ...
        SPARQLAssert.assertUniverses(join);
