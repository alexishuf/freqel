...
import static br.ufsc.lapesd.freqel.query.parse.CQueryContext.annotateTerm;
...
public class QueryOpTest implements TestContext {
    ...
    public void testVarInFilter(BiFunction<CQuery, Set<String>, QueryOp> fac) {
        CQuery query = createQuery(
                Alice, knows, x,
                x, age, y, SPARQLFilterFactory.parseFilter("?y < ?z")
        );
        QueryOp node = fac.apply(query, null);
        assertFalse(node.isProjected());
    ...
    public void testStrictResultVars(BiFunction<CQuery, Set<String>, QueryOp> fac) {
        CQuery query = createQuery(
                ...
                SPARQLFilterFactory.parseFilter("?y < ?z"),
    ...
    public void testCopyPreservesUniverse(BiFunction<CQuery, Set<String>, QueryOp> fac) {
        MutableCQuery query = createQuery(
                Alice, knows, x,
                x, age, u, SPARQLFilterFactory.parseFilter("?u < ?v"), Projection.of("u", "w"));
        ...
        SPARQLAssert.assertUniverses(op);
        Op copy = op.flatCopy();
        assertEquals(copy, op);
        SPARQLAssert.assertUniverses(copy);
    }
    ...
    public void testVarsInOverriddenInputAnnotation(BiFunction<CQuery, Set<String>, QueryOp> fac) {
        CQuery query = createQuery(
                ...
                SPARQLFilterFactory.parseFilter("regex(str(?x1), \"Alice.*\")"),
                y, name, y1,
                AtomInputAnnotation.asRequired(atom3, "a3").override(lit("Bob")).get(),
                SPARQLFilterFactory.parseFilter("regex(str(?y1), \"Bob.*\")")
        );
        QueryOp node = fac.apply(query, null);
    ...
    public void testBindInputInFilterOfQuery(BiFunction<CQuery, Set<String>, QueryOp> fac) {
        CQuery query = createQuery(
                x, age, y,
                SPARQLFilterFactory.parseFilter("?y < ?u"),
    ...
    public void testBindInputInFilter(BiFunction<CQuery, Set<String>, QueryOp> fac) {
        CQuery query = createQuery(
                Alice, knows, x,
                Alice, age, y, SPARQLFilterFactory.parseFilter("?y < ?u"),
                x, age, z, SPARQLFilterFactory.parseFilter("?z < ?y"),
                annotateTerm(u, AtomInputAnnotation.asRequired(atom1, "a1").get())
        );
        QueryOp node = fac.apply(query, null);
        assertEquals(node.getInputVars(), singleton("u"));
        ...
        assertTrue(bound.getQuery().getModifiers().contains(SPARQLFilterFactory.parseFilter("?z < ?y")));
        assertTrue(bound.getQuery().getModifiers().contains(SPARQLFilterFactory.parseFilter("?y < 23")));

        assertEquals(bound.modifiers().filters().size(), 2);
        assertTrue(bound.modifiers().filters().contains(SPARQLFilterFactory.parseFilter("?z < ?y")));
        assertTrue(bound.modifiers().filters().contains(SPARQLFilterFactory.parseFilter("?y < 23")));
    }
}