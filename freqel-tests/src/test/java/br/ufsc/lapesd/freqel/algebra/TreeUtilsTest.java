...
public class TreeUtilsTest implements TestContext {
                        ...
                        .add(ConjunctionOp.builder()
                                .add(UnionOp.builder()
                                        .add(new QueryOp(createQuery(
                                                Alice, knows, x,
                                                x, age, u1, SPARQLFilterFactory.parseFilter("?u1 < 23"))))
                                        .add(new QueryOp(createQuery(
                                                Bob, knows, x,
                                                x, age, u2, SPARQLFilterFactory.parseFilter("?u2 < 23"))))
                                        .build())
                                .add(new QueryOp(createQuery(
                                        x, knows, y,
                                        y, age,   u3, SPARQLFilterFactory.parseFilter("?u3 > 27"))))
                                .build())
                        .add(new QueryOp(createQuery(
                                x, knows, Charlie,
                                x, age, v3,
                                SPARQLFilterFactory.parseFilter("?v3 < 17"))))
                        .build())
                .add(new QueryOp(createQuery(z, age, o3, SPARQLFilterFactory.parseFilter("?o3 > 31"))))
                .build();
    ...
    public void testLeafWithPrefixDict() {
        CQuery qry = createQuery(x, age, u, SPARQLFilterFactory.parseFilter("?u > 23"), StdPrefixDict.DEFAULT);
