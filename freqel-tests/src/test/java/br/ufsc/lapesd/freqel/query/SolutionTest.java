...
public class SolutionTest implements TestContext {
    ...
    public void testEmpty(Supplier<Solution> supplier) {
        ...
        Assert.assertEquals(s.get("x", TestContext.Alice), TestContext.Alice);
        Assert.assertEquals(s.get("missing", TestContext.Alice), TestContext.Alice);
        assertNull(s.get("x"));
    ...
    public void testGet(Supplier<Solution> supplier) {
        Solution s = supplier.get();
        Assert.assertEquals(s.get("x"), TestContext.Alice);
        Assert.assertEquals(s.get("missing", TestContext.Bob), TestContext.Bob);
    ...
    public void testEquals(Supplier<Solution> supplier) {
        MapSolution std = MapSolution.build("x", TestContext.Alice);
    ...
    public void testHashSet(Function<MapSolution, Solution> converter) {
        ...
        for (int i = 0; i < 8; i++) {
            MapSolution solution = MapSolution.builder()
                    .put(TestContext.x, TestContext.Alice)
                    .put(TestContext.y, lit(i)).build();
        ...
        for (int i = 0; i < 1024; i++) {
            MapSolution solution = MapSolution.builder().put(TestContext.x, TestContext.Alice).put(TestContext.y, lit(i)).build();
            expected.add(solution);
            actual.add(converter.apply(solution));
        }
        assertEquals(actual, expected);
    ...
    public void testNotEquals(Supplier<Solution> supplier) {
        MapSolution a = MapSolution.build("y", TestContext.Alice);
        MapSolution b = MapSolution.build("x", TestContext.Bob);
    ...
    public void testNameIsCaseSensitive(Supplier<Solution> supplier) {
        MapSolution a = MapSolution.build("X", TestContext.Alice);
        assertNotEquals(supplier.get(), a);
    }

    @Test(dataProvider = "nonEmptyData")
    public void testMoreVarsNotEquals(Supplier<Solution> supplier) {
        MapSolution a = MapSolution.builder().put("x", TestContext.Alice).put("y", TestContext.Bob).build();
        assertNotEquals(supplier.get(), a);
    }
}