...
public class JoinInfoTest implements TestContext {
    ...
    private static final CQuery xpyfz = createQuery(x, p1, y, SPARQLFilterFactory.parseFilter("?y < ?z"));
    ...
    public void testUseIndexSubsetWithInputs() {
        QueryOp l = new QueryOp(createQuery(
                Alice, knows, x ,
                x,     age,   u , SPARQLFilterFactory.parseFilter("?u < ?v"),
                x,     p1,    o1, SPARQLFilterFactory.parseFilter("?o1 < ?o2")));
        QueryOp r = new QueryOp(createQuery(
                Bob, knows, y ,
                y,   age,   v ,
                y,   p2,    o3, SPARQLFilterFactory.parseFilter("?o3 > ?o4")));
        IndexSet<Triple> triplesUniverse = FullIndexSet.newIndexSet(
                new Triple(Alice, knows, x),
                new Triple(x,     age,   u),
                new Triple(x,     p1,    o1),
