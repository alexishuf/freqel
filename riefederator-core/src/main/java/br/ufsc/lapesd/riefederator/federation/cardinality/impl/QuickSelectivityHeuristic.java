...
public class QuickSelectivityHeuristic implements CardinalityHeuristic {
    private static final Term ground = new StdURI("urn:riefederator:ground");
    private static final Term type = JenaWrappers.fromJena(RDF.type);
    private static final Term sameAs = JenaWrappers.fromJena(OWL2.sameAs);
    private static final ThreadLocal<State> state = ThreadLocal.withInitial(State::new);

    private static int[] GENERIC_COST = {
                     // SPO free
                  1, // 000 ___
                 50, // 001 __O
                 20, // 010 _P_
    ...
    private static class State {
        ...
        private Bitset openStars = new DynamicBitset(), closedStars = new DynamicBitset(),
                       rootStars = new DynamicBitset();
        private CQuery query;
        private IndexSet<Term> starCores = new FullIndexSet<>(16);
        private IndexSet<Term> objects = new FullIndexSet<>(16);
        private List<Bitset> starTriples = new ArrayList<>(32);
        private List<Bitset> obj2Stars = new ArrayList<>(32);
