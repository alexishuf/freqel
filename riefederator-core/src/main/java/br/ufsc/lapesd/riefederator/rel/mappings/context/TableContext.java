package br.ufsc.lapesd.riefederator.rel.mappings.context;
...
class TableContext {
    ...
    addCore(@Nullable MoleculeBuilder b, @Nullable Collection<String> columns) {
        ...
        if (!classes.isEmpty()) {
            MoleculeBuilder atomBuilder = Molecule.builder(tableName + "." + rdfType.getURI())
                    .tag(PostRelationalTag.INSTANCE);
            classes.stream().map(ValueTag::new).forEach(atomBuilder::tag);
            b.out(rdfType, atomBuilder.buildAtom(), PostRelationalTag.INSTANCE);
        }
        ...
        if (!idColumns.isEmpty()) {
            ...
            for (String columnName : idColumns)
                columnsBuilder.add(new Column(tableName, columnName));
            b.tag(new ColumnsTag(columnsBuilder.build(), false));
        }
        return b;
    }
    ...
    public int toRDF(@Nonnull AtomicLong nextId, @Nonnull Model model,
        ...
        if (triples == 0)
            r.addProperty(RDF.type, OWL2.Thing);
