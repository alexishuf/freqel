...
public class JenaVisitor implements QueryVisitor {
    ...
    public JenaVisitor(@Nonnull ConvertOptions convertOptions) {
    ...
    public void visitSelectResultForm(Query query) {
        if (query.isDistinct())
            outerModifiers.add(Distinct.REQUIRED);
        Set<String> vars = query.getProjectVars().stream().map(Var::getVarName).collect(toSet());
        outerModifiers.add(Projection.required(vars));
    }
    @Override
    ...
    public void visitAskResultForm(Query query) {
        outerModifiers.add(Ask.ADVISED);
    }
    @Override
    ...
    public void visitLimit(Query query) {
        if (query.hasLimit())
            outerModifiers.add(Limit.advised((int)query.getLimit()));
    }
    @Override
    ...
    public void finishVisit(Query q) {
        ...
        if (prefixDict != null) {
            TreeUtils.streamPreOrder(op).filter(FreeQueryOp.class::isInstance)
                     .forEach(o -> ((FreeQueryOp) o).getQuery().setPrefixDict(prefixDict));
        }
    }
    ...
    private class ElementVisitor extends ElementVisitorBase {
        private @Nonnull MutableCQuery getMutableCQuery() {
            if (op instanceof FreeQueryOp) {
                ...
                for (ListIterator<Op> it = list.listIterator(list.size()); it.hasPrevious(); ) {
                    ...
                    if (child instanceof FreeQueryOp)
                        return ((FreeQueryOp) child).getQuery();
                }
            }
        ...
        private void saveCQuery(@Nonnull MutableCQuery query) {
            if (op == null) {
                op = new FreeQueryOp(query);
            } else if (op instanceof FreeQueryOp) {
                assert query.containsAll(((FreeQueryOp) op).getQuery())
                        : "Saving query into FreeQueryOp looses triples!";
                ((FreeQueryOp) op).setQuery(query);
            } else if (op instanceof ConjunctionOp) {
                List<Op> list = op.getChildren();
                for (ListIterator<Op> it = list.listIterator(list.size()); it.hasPrevious(); ) {
                    Op previous = it.previous();
                    if (previous instanceof FreeQueryOp) {
                        assert query.containsAll(((FreeQueryOp) previous).getQuery())
                                : "Saving query into old child of ConjunctionOp looses triples";
                        ((FreeQueryOp) previous).setQuery(query);
                        return;
                    }
                }
                ((ConjunctionOp) op).addChild(new FreeQueryOp(query));
            } else {
                op = ConjunctionOp.builder().add(op).add(new FreeQueryOp(query)).build();
            }
        }
        ...
        public void visit(ElementOptional el) {
            if (!convertOptions.getEraseOptionals())
