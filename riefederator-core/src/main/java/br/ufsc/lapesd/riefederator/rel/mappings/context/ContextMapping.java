...
import br.ufsc.lapesd.riefederator.rel.mappings.RelationalMappingUtils;
...
public class ContextMapping implements RelationalMapping {
    ...
    public @Nonnull Term getNameFor(@Nonnull List<Column> columns, @Nonnull List<?> values) {
        String table = RelationalMappingUtils.getTable("getNameFor", logger, columns, values);
        if (table == null) //fallback to a bnode if not killed by Exception
